...
**Speaking:** the mouth is procedural — a syllable-rate oscillator with a
word-level swell and pause gating — driven while the emotion is `"speaking"`.
There is no audio-amplitude analysis; sync with the voice comes from *when*
the speaking state is active (see below), which is gated by real audio
playback. Good enough to read as talking; amplitude-based lip-sync would be
new work.
