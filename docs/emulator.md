...
shows up as a seat card in the `/host` console. Runs as its own service
(`seat-cosimo.…` on the VPS, `:5174` locally) — not inside the CMS.
...
the realtime CORS list must include the emulator's origin (the prod overlay
adds `https://$COSIMO_SEAT_DOMAIN`, dev allows `localhost:5174`).

## Run

```bash
pnpm --filter @cosimo/emulator dev     # :5174, proxies the socket to :4000
