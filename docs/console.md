...
- **Begleiten** (`#begleiten`, `GuidePage.tsx`; `#demo` still lands here) —
  ...
  microphone for talk, an i in a circle for info, sun for light, the
  card reader's arcs around a ring) — never as
...
> 2026-09-22: the outer light had been dark for a day — not a dead route: `in=` is sticky in the LPU-2, the console's cold/warm slider had left pb01 (outer cold white) at `in=0`, and the plain on/off path (the iPad's operator menu, `host:light` / `cabin:light`) only sent `go`. The outer light is off in every scene, and a scene only sends levels for what it switches ON, so nothing ever restored it. Now every switch-on through that path carries a level: the owning fixture's master intensity (so a dimmed cabin stays dimmed), full when that is 0 or unknown — a switch-on can no longer be invisible.
