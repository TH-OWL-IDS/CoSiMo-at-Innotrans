...
import { CircleCheckBig, FlaskConical, Lightbulb, Power, TriangleAlert, X } from "lucide-react";
...
/**
 * "Steuern": the light rig, fixture by fixture — the installer's own test
 * page (MESO, lighting-test.html) rebuilt on our stack. Same controls per
...
export default function LightPanel({ open, onOpenChange, c, cfg, lightOk, riderSection }: {
  open: boolean;
  ...
  // the request log: what went to the LPU-2 from this page, newest first
  const log = useMemo(() => {
    const out: { ts: string; kind: string; control: string; text: string; ok?: boolean }[] = [];
    for (const e of c.logs as LogEvent[]) {
      if (e.kind === "cabin.actuate" && String(e.data.control).startsWith("rig:")) {
        out.push({ ts: e.ts, kind: "→", control: String(e.data.control).slice(4), text: e.data.urls.map((u) => u.replace(/^https?:\/\/[^/]+\/ajax\//, "")).join("  ") });
      } else if (e.kind === "cabin.result" && String(e.data.control).startsWith("rig:")) {
        out.push({ ts: e.ts, kind: "←", control: String(e.data.control).slice(4), text: e.data.ok ? "ok" : e.data.error ?? "fehlgeschlagen", ok: e.data.ok });
      }
    }
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed left-1/2 top-1/2 z-drawer flex h-[min(92vh,980px)] w-[min(1100px,96vw)] -translate-x-1/2 -translate-y-1/2 flex-col gap-3 overflow-hidden rounded-xl border border-line bg-well p-4 shadow-drawer focus:outline-none"
          aria-describedby={undefined}
        >
          <div className="flex flex-wrap items-center gap-2">
            <Dialog.Title className="m-0 mr-auto text-base font-normal">
              <b><Lightbulb size={14} className="-mb-0.5 inline" /> Licht · Steuern</b>{" "}
              <span className={cn(!cfg ? "text-mute" : noAddress ? "text-warn" : lightOk ? "text-ok" : "text-mute")}>
                · {!cfg ? "startet" : noAddress ? "keine LPU-2-Adresse im CMS" : `LPU-2 ${host}${lightOk ? " · ok" : " · nicht bestätigt"}`}
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pr-1">
            ...
            <section className="flex flex-col gap-1.5 rounded-lg border border-line bg-white p-3">
              <span className="text-2xs uppercase tracking-caps text-mute">Anfragen · → gesendet, ← Antwort des iPads</span>
              {log.length === 0 && <span className="text-sm text-mute">noch keine</span>}
              <div className="flex max-h-[180px] flex-col gap-0.5 overflow-y-auto font-mono text-xs">
                {log.map((l, i) => (
                  <div key={i} className={cn("flex gap-2", l.ok === false ? "text-warn" : l.kind === "←" ? "text-ok" : "text-mute")}>
                    <span className="shrink-0 tabular-nums">{new Date(l.ts).toLocaleTimeString("de-DE")}</span>
                    <span className="shrink-0">{l.kind}</span>
                    <span className="shrink-0">{l.control}</span>
