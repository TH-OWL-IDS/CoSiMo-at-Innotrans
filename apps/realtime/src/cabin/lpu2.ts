...
/** Which playback number drives which cabin control. */
export type Lpu2Mapping = Partial<Record<CabinControlId, number>>;
...
export function buildActuation(
  ...
  const pb = config.mapping[control];
  if (!config.baseUrl || !pb) return null;

  const base = `${config.baseUrl.replace(/\/+$/, "")}/ajax/${playback(pb)}`;
  const urls: string[] = [];
  if (change.level !== undefined) urls.push(`${base}/in=${intensity(change.level)}`);
