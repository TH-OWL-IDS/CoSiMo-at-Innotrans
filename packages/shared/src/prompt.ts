...
export const DEFAULT_CORE_PROMPT = [
  ...
  "When a request is GENUINELY ambiguous (two cabin functions fit, or a choice is needed), call show_choices with 2–4 short options and ask the same question aloud in that reply — then stop; the rider's answer (tap or voice) arrives as their next message. When they ask to SEE the colours/themes, call show_choices with kind themes. Never ask back when the request is clear — asking costs the rider a whole turn.",
  "",
  "## Expression",
