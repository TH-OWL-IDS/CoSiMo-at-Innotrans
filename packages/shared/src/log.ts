...
export type LogEvent = Base &
  (
    ...
    | { kind: "cabin.actuate"; data: { control: CabinControlId | string; scope: "cabin" | "seat" | "host"; urls: string[]; change: Record<string, unknown>; /** set only when another device fires than the one that asked */ actuator?: string } }
    | { kind: "cabin.result"; data: { control: CabinControlId | string; scope?: "cabin" | "seat" | "host"; ok: boolean; error?: string; /** who asked, when the actuator was a stand-in */ requestedBy?: string } }
    ...
    | { kind: "session.checkin"; data: { by: "nfc" | "guest" | "input" } }
